...
function WeekColumn({ title, m, highlights }: WeekColumnProps) {
  ...
  return (
    <div className="rounded-lg p-3 space-y-1.5" style={{
      ...
      <p className="text-xs font-semibold mb-2" style={{ color: '#1E6F70', fontFamily: 'Syne' }}>{title}</p>
...
function DiagnosisField({ label, value, color, accent }: { label: string; value: string; color: string; accent?: boolean }) {
  return (
    <div className="rounded-lg p-3" style={{
      background: accent ? 'rgba(30,111,112,0.06)' : 'rgba(255,255,255,0.02)',
      border: `1px solid ${accent ? 'rgba(30,111,112,0.2)' : 'rgba(255,255,255,0.06)'}`,
    }}>
      <p className="text-xs font-semibold mb-1" style={{ color, fontFamily: 'Syne' }}>{label}</p>
      <p className="text-sm leading-relaxed" style={{ color: '#E6DCC8' }}>{value || '—'}</p>
    </div>
  );
}
