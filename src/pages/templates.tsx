    ...
    <div className="flex-1 flex flex-col min-h-0">
      ...
      <div className="flex-1 overflow-y-auto px-4 md:px-8 py-6">
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="glass-card p-4">
              <div className="flex items-center gap-2 mb-2">
                <Zap size={14} color="#3A8FA3" />
                <p className="text-xs" style={{ color: '#6E7F86' }}>Total Templates</p>
              </div>
              <p className="text-2xl font-bold" style={{ color: '#3A8FA3', fontFamily: 'Syne' }}>
                {templates.length}
              </p>
            </div>
            <div className="glass-card p-4">
              <div className="flex items-center gap-2 mb-2">
                <TrendingUp size={14} color="#1E6F70" />
                <p className="text-xs" style={{ color: '#6E7F86' }}>Avg Response</p>
              </div>
              <p className="text-2xl font-bold" style={{ color: '#1E6F70', fontFamily: 'Syne' }}>
                {avgResponseRate}%
              </p>
            </div>
            <div className="glass-card p-4">
              <div className="flex items-center gap-2 mb-2">
                <TrendingUp size={14} color="#1E6F70" />
                <p className="text-xs" style={{ color: '#6E7F86' }}>Avg Open Rate</p>
              </div>
              <p className="text-2xl font-bold" style={{ color: '#1E6F70', fontFamily: 'Syne' }}>
                42%
              </p>
          ...
          {/* Form */}
          {showForm && (
            <div className="glass-card p-5 animate-fadeUp" style={{ border: '1px solid rgba(58,143,163,0.2)' }}>
              <h3 className="text-sm font-semibold mb-4" style={{ color: '#1E6F70' }}>
              ...
              <div className="space-y-4 mb-4">
                ...
                <div>
                  ...
                  />
                  <p className="text-xs mt-1.5" style={{ color: '#6E7F86' }}>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {(['professional', 'casual', 'value-driven', 'problem-solving'] as ToneType[]).map((tone) => (
                      <button
                        ...
                        style={{
                          background: form.tone === tone ? 'rgba(58,143,163,0.2)' : 'rgba(255,255,255,0.04)',
                          border: `1px solid ${form.tone === tone ? 'rgba(58,143,163,0.4)' : 'rgba(255,255,255,0.1)'}`,
                          color: form.tone === tone ? '#1E6F70' : '#6E7F86',
                        }}
                      >
                        {tone}
                      </button>
          ...
          {templates.length === 0 && !showForm ? (
            <div className="flex flex-col items-center justify-center py-16">
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-4" style={{ background: 'rgba(58,143,163,0.1)' }}>
                <Zap size={28} color="#3A8FA3" />
              </div>
                ...
                <div key={template.id} className="glass-card p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="tag text-xs" style={{ fontSize: '9px', padding: '2px 6px', background: 'rgba(58,143,163,0.15)', color: '#1E6F70' }}>
                          {template.tone}
                        </span>
                        {template.responseRate && (
                          <span className="text-xs" style={{ color: '#1E6F70', fontWeight: 600 }}>
                            {template.responseRate}% response
                          </span>
                        )}
                      </div>
                      <div className="flex items-center gap-2 text-xs" style={{ color: '#6E7F86' }}>
                    ...
                    <div className="flex items-center gap-2">
                      <button onClick={() => handleEdit(template)} className="p-2 rounded-lg" style={{ background: 'rgba(58,143,163,0.1)' }} title="Edit">
                        <Edit2 size={12} color="#1E6F70" />
                      </button>
                      <button onClick={() => handleCopy(template.body)} className="p-2 rounded-lg" style={{ background: 'rgba(58,143,163,0.1)' }} title="Copy">
                        <Copy size={12} color="#1E6F70" />
                      </button>
                      <button onClick={() => handleDelete(template.id)} className="p-2 rounded-lg" style={{ background: 'rgba(176,67,42,0.1)' }} title="Delete">
                  ...
                  {template.subject && (
                    <div className="mb-2 p-2 rounded" style={{ background: 'rgba(255,255,255,0.03)' }}>
                      <p className="text-xs font-medium mb-0.5" style={{ color: '#6E7F86' }}>Subject:</p>
